...
pub fn detect_usb_drive() -> Option<String> {
            ...
            for user_entry in user_entries.flatten() {
                ...
                if let Ok(device_entries) = fs::read_dir(&user_path) {
                    for device_entry in device_entries.flatten() {
                        ...
                        // Return the mount point directory itself, not a file inside it
    ...
    #[cfg(target_os = "macos")]
    {
        // /Volumes contains the system disk (Macintosh HD) — skip it
        ...
        if let Ok(entries) = fs::read_dir("/Volumes") {
            for entry in entries.flatten() {
                ...
                if SYSTEM_VOLUMES.iter().any(|s| name_str.eq_ignore_ascii_case(s)) {
                    continue;
                }
                // Skip hidden volumes
    ...
    if let Ok(entries) = fs::read_dir(&pending_dir) {
        for entry in entries.flatten() {
            ...
            if path.extension().and_then(|e| e.to_str()) == Some("json") {
                ...
                // Store the full path as id so delete_pending_transaction can find the file
...
pub fn delete_pending_transaction(usb_path: &str, tx_id: &str) -> Result<(), String> {
    ...
    // tx_id is now the full file path
    ...
    // Silently succeed if already deleted (idempotent)
    Ok(())
}
