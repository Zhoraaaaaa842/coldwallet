...
pub async fn get_balance(rpc_url: &str, address: &str) -> Result<String, String> {
    ...
    let balance_hex = result.get("result").and_then(|v| v.as_str())
        .ok_or("Invalid response format")?;
    // Use u128 to avoid overflow (u64 overflows at ~18.4 ETH)
...
pub async fn get_nonce(rpc_url: &str, address: &str) -> Result<u64, String> {
    ...
    let nonce_hex = result.get("result").and_then(|v| v.as_str())
        .ok_or("Invalid response format")?;
...
pub async fn fetch_eth_price_rub() -> Result<f64, String> {
    ...
    let response = client.get("https://api.coingecko.com/api/v3/simple/price")
        .query(&[("ids", "ethereum"), ("vs_currencies", "rub")])
        .send().await
        .map_err(|e| format!("Request failed: {}", e))?;
    let result: serde_json::Value = response.json().await
        .map_err(|e| format!("Failed to parse response: {}", e))?;
    let price = result.get("ethereum").and_then(|v| v.get("rub"))
        .and_then(|v| v.as_f64()).ok_or("Failed to parse price")?;
    Ok(price)
}
...
pub async fn get_transaction_history(_rpc_url: &str, address: &str) -> Result<Vec<serde_json::Value>, String> {
    ...
    let client = reqwest::Client::new();
    let response = client.get(&etherscan_url).send().await
        .map_err(|e| format!("Request failed: {}", e))?;
    let result: serde_json::Value = response.json().await
        .map_err(|e| format!("Failed to parse response: {}", e))?;

    // Etherscan returns status "0" with "No transactions found" — treat as empty, not error
    let status = result.get("status").and_then(|v| v.as_str()).unwrap_or("0");
    if status != "1" {
        let msg = result.get("message").and_then(|v| v.as_str()).unwrap_or("");
        if msg.contains("No transactions") || msg == "No transactions found" {
            return Ok(vec![]);
        }
        return Err(msg.to_string());
    }

    let txs = result.get("result").and_then(|v| v.as_array())
        .ok_or("Invalid response format")?;
    ...
    for tx in txs {
        ...
        let tx_type = if from.to_lowercase() == address.to_lowercase() { "outgoing" } else { "incoming" };
        transactions.push(serde_json::json!({
            "hash": tx.get("hash").and_then(|v| v.as_str()).unwrap_or(""),
            "from": from, "to": to, "value": value,
            "nonce": tx.get("nonce").and_then(|v| v.as_str()).unwrap_or("0"),
            "gasLimit": tx.get("gas").and_then(|v| v.as_str()).unwrap_or("21000"),
            "gasUsed": tx.get("gasUsed").and_then(|v| v.as_str()).unwrap_or("0"),
            "gasPrice": tx.get("gasPrice").and_then(|v| v.as_str()).unwrap_or("0"),
            "blockNumber": tx.get("blockNumber").and_then(|v| v.as_str()).unwrap_or("0"),
            "timestamp": timestamp,
            "status": if tx.get("isError").and_then(|v| v.as_str()) == Some("0") { "confirmed" } else { "failed" },
            "type": tx_type,
        }));
    }
    Ok(transactions)
...
pub async fn fetch_gas_settings(rpc_url: &str) -> Result<serde_json::Value, String> {
    ...
    let gas_price_hex = result.get("result").and_then(|v| v.as_str())
        .ok_or("Invalid response format")?;
...
pub async fn broadcast_transaction(rpc_url: &str, raw_tx: &str) -> Result<serde_json::Value, String> {
    ...
    let tx_hash = result.get("result").and_then(|v| v.as_str())
        .ok_or("Invalid response format")?;
    Ok(serde_json::json!({ "hash": tx_hash }))
}
