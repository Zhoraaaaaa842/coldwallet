<template>
  ...
  >
    ...
    <div 
      ...
      :class="transaction.type === 'incoming' ? 'bg-success/20' : 'bg-error/20'"
    >
      <svg v-if="transaction.type === 'incoming'" class="w-5 h-5 text-success" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="text-right">
      <p 
        class="font-semibold"
        :class="transaction.type === 'incoming' ? 'text-success' : 'text-error'"
      >
        {{ transaction.type === 'incoming' ? '+' : '-' }}{{ formatEth(transaction.value) }} ETH
      </p>
